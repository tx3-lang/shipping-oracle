...
[tx3](https://github.com/tx3-lang/tx3) protocol describing the Cardano transactions used by the **pull-based** Shipping Oracle plus the Milestone 3 ADA escrow template.

## Overview

```
publish_scripts          (one-time, signed by Oracle)
   └─► publishes governance_nft + oracle + escrow as reference scripts
...
lock_escrow_ada          (per order, signed by Buyer)
   └─► locks ADA at the escrow validator with order + shipment hashes

release_escrow           (after delivery, signed by Merchant)
   └─► spends escrow to Merchant when the oracle signs DELIVERED

refund_escrow            (after timeout, signed by Buyer)
   └─► spends escrow back to Buyer after refund_after
```
...
- `main.tx3` — protocol definition: parties, env, oracle datums/redeemers, and escrow datums/redeemers.
...
| `Buyer` | `lock_escrow_ada`, `refund_escrow` | locks ADA and reclaims it after timeout |
| `Merchant` | `release_escrow` | collects escrow when oracle attests delivery |
...
| `BUYER` (party address) | buyer wallet's bech32 address | before escrow txs |
| `MERCHANT` (party address) | merchant wallet's bech32 address | before escrow txs |
...
| `ESCROW_SCRIPT` | `plutus.json → escrow.spend.compiledCode` | after on-chain build |
| `ESCROW_SCRIPT_REF` | `<publish_tx_hash>#<idx>` of the escrow reference-script output | after `publish_scripts` |
...
> `policy Escrow` currently pins the compiled escrow script hash directly in `main.tx3`, matching the upstream e-commerce template style. If `escrow.ak` changes, rebuild `onchain/plutus.json` and update the `Escrow` policy hash in `main.tx3`.
...
- **Escrow flow** — see `TESTING.md` section 5.9. The Rust SDK example `e2e_escrow_flow` writes tx3 args JSON for `lock_escrow_ada`, then for `release_escrow` and `refund_escrow` once you provide the locked escrow UTxO ref.
